...
from app.models.TranslationJob import TranslationJob
...
def handle_jobs():
    ...
    try:
        ...
        thread = threading.Thread(target=process_translation_job, args=(job.id, current_app._get_current_object()))
        thread.start()
        ...
        try:
            ...
            for section in latest_version.sections:
                ...
                if section.description:
                    translatable_items[f"section_{section.id}_desc"] = section.description
                    
                for question in section.questions:
                    translatable_items[f"question_{question.id}_label"] = question.label
                    if question.help_text:
                        translatable_items[f"question_{question.id}_help"] = question.help_text
                    if question.placeholder:
                        translatable_items[f"question_{question.id}_place"] = question.placeholder
                        
                    for option in question.options:
                        translatable_items[f"option_{option.id}_label"] = option.option_label
            ...
            for i, lang in enumerate(job.target_languages):
                ...
                try:
                    translated_dict = AIService.translate_bulk(translatable_items, job.source_language, lang)
