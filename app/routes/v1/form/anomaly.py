...
@anomaly_bp.route('/<form_id>/detect-anomalies/batch/<batch_id>/status', methods=['GET'])
@jwt_required()
def get_batch_scan_status(form_id: str, batch_id: str):
    ...
    # Get batch status
    status = AnomalyDetectionService.get_batch_status(batch_id=batch_id, nocache=nocache)
