...
from app.models.Form import CustomFieldTemplate, Question, Form
...
def list_field_templates():
    try:
        ...
        result = []
        for t in templates:
            data = dict(t.data) if hasattr(t, 'data') and t.data else \
                   (t.question_data.to_mongo().to_dict() if hasattr(t, 'question_data') and t.question_data else {})
...
def save_field_template():
    try:
        ...
        template = CustomFieldTemplate(
            ...
            data=template_data if template_data else (question_data_raw if template_type == 'question' else {})
        )
...
def get_field_template(template_id):
    try:
        current_user_id = get_jwt_identity()
        t = CustomFieldTemplate.objects.get(id=template_id, user_id=str(current_user_id))
