...
def has_form_permission(user, form, action):
    ...
    def is_in_list(target_list):
        ...
        if user_id_str in target_list:
            return True
        # Check if any of user's roles are in list
        for role in user_roles:
            if role in target_list:
                return True
        return False

    policy = form.access_policy if hasattr(form, 'access_policy') and form.access_policy else None
        ...
        if policy:
            ...
            if policy.form_visibility == 'restricted':
                if user_dept and user_dept in (policy.allowed_departments or []):
                    return True
                if is_in_list(policy.can_view_responses): # If you can see responses, you can see the form
                    return True
        return False
    ...
    if action == "edit" or action == "edit_design":
    ...
    if action == "manage_access":
        if user.is_admin_check(): # Admins can always manage
            return True
