...
def get_analytics_summary(form_id):
    try:
        ...
        last_submitted_at = last_submission.submitted_at.isoformat() if last_submission else None
    ...
    try:
        ...
        for section in latest_version.sections:
            ...
            for q in section.questions:
                if q.field_type in target_types:
                    choice_questions[str(q.id)] = {"label": q.label, "sid": sid, "type": q.field_type}
        ...
        for r in responses:
            for qid, info in choice_questions.items():
                ...
                entries = section_data if isinstance(section_data, list) else [section_data]
...
def get_full_analytics(form_id):
    ...
    try:
            ...
            for r in responses:
                for qid, info in q_map.items():
                    ...
                    for entry in entries:
                        ...
                        if isinstance(val, list):
                            for v in val: distribution[info["label"]][str(v)] += 1
                        else:
            ...
            field_dist_map[label] = [
                {
                    "label": k, 
                    "count": v, 
                    "percentage": round((v / total_answers) * 100, 1) if total_answers > 0 else 0
                }
        ...
        return jsonify({
            ...
            "completionRate": 0.0 if total == 0 else 0.85, # Mocked as requested
            "trends": trends,
