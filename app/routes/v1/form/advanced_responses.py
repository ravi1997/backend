...
def fetch_response_meta(form_id):
    ...
    try:
        ...
        draft_responses = FormResponse.objects(form=form.id, deleted=False, is_draft=True).count()
        submitted_responses = total_responses - draft_responses
        
        # Last submission
        last_response = FormResponse.objects(form=form.id, deleted=False).order_by("-submitted_at").first()
        ...
        return jsonify({
            ...
            "last_submission": last_response.submitted_at if last_response else None
        }), 200
...
def get_form_access_control(form_id):
    ...
    try:
        ...
        permissions = {
            ...
            "delete_responses": has_form_permission(current_user, form, "delete_responses"),
        ...
        return jsonify({
            ...
            "policy_summary": {
                "visibility": policy.form_visibility if policy else "legacy",
                "response_scope": policy.response_visibility if policy else "legacy",
                "allowed_departments": policy.allowed_departments if policy else []
            } if policy else None
        }), 200
...
def update_access_policy(form_id):
    ...
    try:
        ...
        if not has_form_permission(current_user, form, "manage_access"):
            return jsonify({"error": "Unauthorized to manage access for this form"}), 403
        ...
        if not form.access_policy:
            from app.models.Form import AccessPolicy
            form.access_policy = AccessPolicy()
            
        # Update fields
        fields = [
            'can_view_responses', 'can_edit_responses', 'can_delete_responses',
            'response_visibility', 'can_create_versions', 'can_edit_design',
            'can_clone_form', 'can_manage_access', 'can_view_audit_logs',
            'can_delete_form', 'form_visibility', 'allowed_departments'
        ]
