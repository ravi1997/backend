...
def register():
    ...
    try:
        ...
        if user.employee_id and User.objects(employee_id=user.employee_id).first():
...
def login():
    ...
    # --- EMPLOYEE LOGIN ---
    if password:
        identifier = next((data.get(field) for field in identifier_fields if data.get(field)), None) or data.get('identifier')
    ...
    if request.headers.get("HX-Request"):
            ...
            """
                ...
                <script>
                    setTimeout(function () {
                        ...
                        `;
                        setTimeout(function () {
                            window.location.href = "a59385dc-45dc-47cf-ad99-ec000695fb48";
                        }, 1000);
                    }, 2000);
                </script>
        ...
        set_access_cookies(resp, access_token)
        current_app.logger.info(f"Returning HTMX response for user {user.username}")
        return resp

    current_app.logger.info(f"Returning JSON response for user {user.username}")
    return resp, 200
